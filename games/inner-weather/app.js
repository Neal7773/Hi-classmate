(() => {
  'use strict';
  const data = WEATHER_DATA, $ = id => document.getElementById(id);
  const url = 'https://hiclassmate.com/games/inner-weather/';
  ...
  function readingCard(title, text, match) {
    ...
    if (match) card.append(Object.assign(document.createElement('span'), { className: 'weather-match', textContent: match }));
  ...
  function result() {
    ...
    current = WeatherEngine.calculate(data, choices);
    ...
    $('chemistry').replaceChildren(...r.chemistry.map((item, i) => readingCard(i ? 'Interesting Opposite' : 'Easy Weather Match', item.text, item.title)));
    progress(8, 'Your emotional forecast', '8 / 8');
  ...
  function reset(focus = false) {
    ...
    progress(0, 'Your emotional forecast', '8 choices');
  ...
  $('next-game-link').onclick = () => track('next_game_click', { ...resultParams(), next_game_id: 'what_kind_of_overthinker' });
