(() => {
  'use strict';
  const data = DATING_DATA;
  const gameUrl = 'https://hiclassmate.com/games/toxic-dating-habit/';
  ...
  function finish() {
    current = DatingEngine.calculate(data, choices);
    ...
    $('result-habit').textContent = result.habit;
    $('result-impact').textContent = result.impact;
    $('result-repair').textContent = result.repair;
    $('trait-list').replaceChildren(...result.traits.map(text => Object.assign(document.createElement('span'), { textContent: text })));
    progress(8, 'Your dating habit, exposed');
    show('result');
  ...
  function share(resultShare) {
    ...
    const quote = resultShare ? `I got ${result.title}. My habit? ${result.habit} What is YOUR toxic dating habit?` : 'What is your most toxic dating habit? 8 questions. 6 habits. Be honest.';
  ...
  function reset(focus = false) {
    ...
    progress(0, 'No more “it’s not me.”');
    show('start');
  ...
  $('next-game-link').addEventListener('click', () => track('next_game_click', { destination_game_id: 'villain_era' }));
  track('game_view');
  reset();
})();
