...
13. New mobile game screens should prioritize instant scanning: make the active question visually dominant, keep the combined answer area around one-quarter to one-third of the viewport instead of stretching to fill available height, and let the question-state card size itself to its content. Compress results around one prominent score/percentage plus short keyword chips. Avoid long result paragraphs or footer disclaimers inside the interactive result card; keep necessary context in the supporting page copy.
